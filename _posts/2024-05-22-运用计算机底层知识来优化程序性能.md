...
![](/assets/images/result_compare_combine1_with_op.png){: width="472" height="129" }
...
![](/assets/images/result_compare_combine1_with_combine2.png){: width="472" height="129" }
...
![](/assets/images/result_compare_combine2_with_combine3.png){: width="472" height="129" }
...
![](/assets/images/result_compare_combine3_with_combine4.png){: width="472" height="129" }
...
![](/assets/images/block_diagram_of_an_out_of_order_processor.png){: width="472" height="129" }
...
![](/assets/images/core_i7_haswell_performance_of_the_arithmetic_op.png){: width="472" height="129" }
...
![](/assets/images/result_compare_combine4_bottleneck.png){: width="472" height="129" }
...
![](/assets/images/graphical_representation_of_inner_loop_code_combine4.png){: width="472" height="129" }
...
![](/assets/images/abstracting_combine4_operations_as_a_data_flow_graph.png){: width="472" height="129" }

这张图中的白色方框中的操作是可以被忽略的，一个是比较操作 `cmp`，另一个是跳转操作 `jne`，这两个操作不会设计到算术运算或是操作内存，并不是影响程序性能的主要因素，这里忽略不计，于是我们还可以继续简化：

![](/assets/images/abstracting_combine4_operations_as_a_data_flow_graph2.png){: width="472" height="129" }

这里的依赖关系就非常明显了，并且结合每个操作所需要的时钟周期，我们可以清楚地发现 `mul` 操作是瓶颈，每次循环的时间是由 `mul` 决定的，我们还可以从整个循环来看：

![](/assets/images/abstracting_combine4_operations_as_a_data_flow_graph3.png){: width="472" height="129" }
...
![](/assets/images/abstracting_combine5_operations_as_a_data_flow_graph.png){: width="472" height="129" }

即使把图中的 `mul` 换成 `add`，瓶颈（Critical Path）依然在 `add` 上，这里不存在 `load` 不过来的情况。结果比较如下：

![](/assets/images/result_compare_combine4_with_combine5.png){: width="472" height="129" }
...
![](/assets/images/result_compare_combine5_with_combine6.png){: width="472" height="129" }

同样的，我们也可以得出依赖图：

![](/assets/images/abstracting_combine6_operations_as_a_data_flow_graph.png){: width="472" height="129" }

你可能会说，这也没有达到吞吐量的上限，是不是我们还有优化的空间？是的，想要增加吞吐量，就必须不间断地让执行单元执行，而上面 `2 X 2` 其实还不够，我们需要把这里的 `2` 变成 `k`，这个系数 `k` 越大，就越接近上限：

![](/assets/images/cpe_performance_of_k_x_k_loop_unrolling.png){: width="472" height="129" }
...
![](/assets/images/abstracting_combine7_operations_as_a_data_flow_graph.png){: width="472" height="129" }
...
![](/assets/images/result_compare_combine6_over_op.png){: width="472" height="129" }
...
![](/assets/images/write_and_read_memory_locations_eg.png){: width="472" height="129" }

结果不一样是显而易见的，但是这里我们好奇的是上面两个例子的运行效率，例子 A 的 CPE 是 1.3，而例子 B 的 CPE 则是 7.3，因为例子 B 中的存取存在依赖关系。

为了进一步阐述这里面的关系，我们需要了解存取单元之间的关系：

![](/assets/images/detail_of_load_and_store_units.png){: width="472" height="129" }
...
![](/assets/images/data_flow_representation_of_function_write_read.png){: width="472" height="129" }
