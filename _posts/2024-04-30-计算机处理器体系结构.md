...
![](/assets/img/images/y86_64_function_codes.png){: width="472" height="129" }
...
![](/assets/img/images/y86_64_complete_ISA.png){: width="472" height="129" }
...
![](/assets/img/images/logic_gate_sample.png){: width="472" height="129" }

与、或、非就是三个最为基础，也最为简单的门电路，我们可以将其组合得到我们想要的逻辑和运算符，比如 **相等**：

![](/assets/img/images/logic_gate-eq.png){: width="472" height="129" }
...
![](/assets/img/images/logic_gate-multiplexor.png){: width="472" height="129" }
...
![](/assets/img/images/logic_gate-multi_eq.png){: width="472" height="129" }

下面这个是 8 字节的选择器：

![](/assets/img/images/logic_gate-multi_multiplexor.png){: width="472" height="129" }

这里的 HCL 是一个 case 语句，表示的意思是，如果 s 不为 0，选择 A，否则的话选择 B。

类似上面这种电路被称为组合电路，最为著名的组合电路是 **算术逻辑单元（ALU，Arithmetic/logic unit）**：

![](/assets/img/images/logic_gate-alu.png){: width="472" height="129" }
...
![](/assets/img/images/clock_register.png){: width="472" height="129" }
...
![](/assets/img/images/memory_register_example.png){: width="472" height="129" }
...
![](/assets/img/images/computations_in_seq_y86_instructions_opq_rrmovq_irmovq.png){: width="472" height="129" }
...
![](/assets/img/images/computations_in_seq_y86_instructions_pushq_popq.png){: width="472" height="129" }
...
![](/assets/img/images/abstract_view_of_seq.png){: width="472" height="129" }

注意，上面这只是逻辑图，并没有涉及硬件。下面这张才是一个初步的硬件架构图：

![](/assets/img/images/hardware_structure_seq.png){: width="472" height="129" }
...
![](/assets/img/images/seq_fetch_stage.png){: width="472" height="129" }
...
![](/assets/img/images/seq_decode_stage.png){: width="472" height="129" }
...
![](/assets/img/images/seq_execute_stage.png){: width="472" height="129" }
...
![](/assets/img/images/seq_memory_stage.png){: width="472" height="129" }
...
![](/assets/img/images/seq_pc_update_stage.png){: width="472" height="129" }
...
![](/assets/img/images/unpipelined_computation_hardware.png){: width="472" height="129" }
...
![](/assets/img/images/three_stage_pipelined_computation_hardware.png){: width="472" height="129" }

吞吐量计算为 `1 instruction / (100 + 20) ps * (1000 ps / 1 ns) = 8.33 GIPS`，这明显比之前的吞吐量要大许多。当然了，每个阶段的执行都是时钟信号在控制的：

![](/assets/img/images/three_stage_pipeline_timing.png){: width="472" height="129" }
...
![](/assets/img/images/limitation_of_pipelining.png){: width="472" height="129" }
...
![](/assets/img/images/hardware_structure_of_pipe-.png){: width="472" height="129" }
...
![](/assets/img/images/pipelined_execution_of_prog4_without_special_control.png){: width="472" height="129" }

很明显，第三条指令依赖于头两条指令，如果不做任何处理的话会发生错误。我们在第二条指令和第三条指令插入三个空指令 `nop`：

![](/assets/img/images/pipelined_execution_of_prog1_without_special_control.png){: width="472" height="129" }
...
![](/assets/img/images/pipelined_execution_of_prog4_using_stalls.png){: width="472" height="129" }
...
![](/assets/img/images/pipelined_execution_of_prog4_using_forwarding.png){: width="472" height="129" }
...
![](/assets/img/images/example_of_load_use_data_hazard.png){: width="472" height="129" }
...
![](/assets/img/images/handle_load_use_data_hazard_by_stalling.png){: width="472" height="129" }

另外，我们之前也说了，`ret` 指令和 `jXX` 指令情况特殊，前者返回地址不确定，后者其实猜的，对不对还不知道，我们还需要考虑这两种情况。

对于 `ret` 指令来说，我们可以将其后面的指令推迟执行，直到返回地址明确：

![](/assets/img/images/simplified_view_of_ret_instruction_processing.png){: width="472" height="129" }

如果条件判断猜测失败了怎么办？从电路设计图中可以看到，条件码（CC）是在执行阶段产生的，当条件码确定时，我们才会知道我们之前的猜测是对还是错，如果是对的就继续执行，如果是错的话，正在执行的指令需要被丢弃，类似下面这样：

![](/assets/img/images/processing_mispredicted_branch_instructions.png){: width="472" height="129" }
...
![](/assets/img/images/pipe_pc_selection_and_fetch_logic.png){: width="472" height="129" }
...
![](/assets/img/images/pipe_decode_and_write_back_stage_logic.png){: width="472" height="129" }
...
![](/assets/img/images/pipe_execute_stage_logic.png){: width="472" height="129" }
...
![](/assets/img/images/pipe_memory_stage_logic.png){: width="472" height="129" }
...
![](/assets/img/images/additional_pipeline_register_operation.png){: width="472" height="129" }

第一种就是正常情况，当时钟信号升高，寄存器以及输出都变为输入。第二种是维持不变，也就是我们之前所说的推迟执行（stall），第三种是将寄存器清空，也就是取消指令的阶段执行。

现在回过头看除了异常之外的三种特殊情况，每种情况控制单元需要做的事情如下：

![](/assets/img/images/actions_for_pipeline_control_logic.png){: width="472" height="129" }

但是我们也说过，因为不同指令可能在不同阶段同时被执行，我们还需要考虑组合的情况：

![](/assets/img/images/pipeline_states_for_special_control_conditions.png){: width="472" height="129" }

比如这里的组合 A 就是当执行阶段发生了预测错误，解码阶段解码了 `ret` 指令，我们需要把这两个阶段放到一起考虑：

![](/assets/img/images/ret_and_mispredicted_combination.png){: width="472" height="129" }

组合 B 也是同样的道理：

![](/assets/img/images/ret_and_load_use_hazard_combination.png){: width="472" height="129" }

最后，我们可以把我们的控制单元加到之前的电路中，控制单元主要是控制时钟寄存器，这里就略去之前的模块：

![](/assets/img/images/pipe_pipeline_control_logic.png){: width="472" height="129" }
